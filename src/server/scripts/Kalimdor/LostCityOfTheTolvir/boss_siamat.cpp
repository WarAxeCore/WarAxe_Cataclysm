...
public:
	...
	struct boss_siamatAI : public ScriptedAI
	{
		...
		void JustDied(Unit* /*killer*/)
		{
			...
			me->BossYell("The sky... Beckons...", 20226);
			summons.DespawnAll();
		...
		void KilledUnit(Unit* victim)
		{
			if (victim->GetTypeId() == TYPEID_PLAYER)
				me->BossYell("Nothing more than dust in the wind.", 20232);
		}
		...
		void SummonedCreatureDespawn(Creature* summon)
		{
			if (summon->GetEntry() == NPC_SERVANT_OF_SIAMAT)
				instance->SendEncounterUnit(ENCOUNTER_FRAME_REMOVE, summon);
		}
		...
		void UpdateAI(const uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_STORM_BOLT_RANDOM:
					DoCastRandom(SPELL_STORM_BOLT_RANDOM, 100.0f);
				...
				case EVENT_SUMMON_NPC_MINION_OF_SIAMAT:
					me->BossYell("Cower before the tempest storm!", 20228);
				...
				case EVENT_CLOUD_BURST_SUMMON:
					DoCastRandom(SPELL_CLOUD_BURST_SUMMON, 100.0f);
...
public:
	...
	struct npc_wind_tunnelAI : public PassiveAI
	{
		...
		void PassengerBoarded(Unit* who, int8 seatId, bool apply)
		{
			...
			who->_Events.AddEvent(new jumpDest(who), who->_Events.CalculateTime(2000));
		}

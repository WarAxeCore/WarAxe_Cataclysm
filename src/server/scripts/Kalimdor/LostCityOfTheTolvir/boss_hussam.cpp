/*
 * Copyright (C) 2020 WarAxeCore Team <https://github.com/Razfury/WarAxeCore>
 *
...
public:
	...
	bool Execute(uint64 execTime, uint32 /*diff*/)
	{
		...
		_caster->_Events.AddEvent(new SummonShockwaveStalker(_caster), _caster->_Events.CalculateTime(1000));
		return false;
	}
...
public:
	...
	struct boss_general_husamAI : public ScriptedAI
	{
		...
		void EnterCombat(Unit* /*who*/)
		{
			...
			me->BossYell("Invaders! You shall go no further!", 21886);
				...
				case EVENT_SHOCKWAVE:
				{
					...
					if (!creatures.empty())
					{
						for (std::list<Creature*>::iterator iter = creatures.begin(); iter != creatures.end(); ++iter)
						{
							...
							(*iter)->_Events.AddEvent(new SummonShockwaveStalker((*iter)), (*iter)->_Events.CalculateTime(1000));
						}
					}
...
public:
	...
	class spell_bad_intentions_effect_SpellScript : public SpellScript
	{
		...
		void HandleScript(SpellEffIndex effIndex)
		{
			...
			if (!triggers.empty())
			{
				triggers.sort(SkyFire::ObjectDistanceOrderPred(GetHitUnit(), true));

...
public:
	...
	struct boss_high_prophet_barimAI : public ScriptedAI
	{
		...
		void EnterCombat(Unit* /*who*/)
		{
			...
			me->BossYell("Begone infidels, you are not welcome here!", 19735);
		...
		void KilledUnit(Unit* who)
		{
			...
			{
				uint32 randomTalk = urand(1, 2);
				if (randomTalk == 1)
				{
					me->BossYell("May peace find you now.", 19738);
				}
				else
				{
					me->BossYell("The Heavens take you!", 19736);
				}
			}
		}
		...
		{
			me->BossYell("Death is only the beginning!", 19733);
			instance->SendEncounterUnit(ENCOUNTER_FRAME_REMOVE, me);
			...
			if (soulCounter >= 3 && IsHeroic())
			{
				...
				if (!playerList.isEmpty())
					for (Map::PlayerList::const_iterator itr = playerList.begin(); itr != playerList.end(); ++itr)
						...
						{							
							players->CompletedAchievement(sAchievementMgr->GetAchievement(ACHIEVEMENT_KILL_IT_WITH_FIRE));
						}
			}
			DespawnCreatures(48904);
			DespawnCreatures(48907);
		...
		void JustSummoned(Creature* summon)
		{
			...
			case 48904: // blaze egg
			{
				...
				summon->_Events.AddEvent(new EggEvent(summon), summon->_Events.CalculateTime(4000));
				break;
			}
			...
			case NPC_VEIL_OF_TWILIGHT:
				...
				summon->_Events.AddEvent(new WailofDarknessEvent(summon), summon->_Events.CalculateTime(2000));
				break;
			}
		}
		...
		void UpdateAI(const uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_FENIX_SUMMON:
					DoCastRandom(SPELL_BLAZE_OF_THE_HEAVENS_SUMMON_EGG, 100.0f);
					break;
				...
				case EVENT_REPENTANCE:
					me->BossYell("Kneel before me and repent!", 19737);
					me->StopMoving();
...
public:
	...
	class spell_fifty_lashings_AuraScript : public AuraScript
	{
		...
		void OnProc(AuraEffect const* /*aurEff*/, ProcEventInfo& /*eventInfo*/)
		{
			...
			GetCaster()->_Events.AddEvent(new fiftyLashingsProcDelay(GetCaster()), GetCaster()->_Events.CalculateTime(1000));
		}

...
public:
	...
	struct boss_ozrukAI : public BossAI
	{
		...
		{
			me->BossYell("None may pass into the World's Heart!", 21919);
		...
		void KilledUnit(Unit * victim)
		{
			if (victim->GetTypeId() == TYPEID_PLAYER)
				me->BossYell("The cycle is complete.", 21920);
		}
		...
		void UpdateAI(uint32 const diff)
		{
			...
			if (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_SPIKE_SHIELD:
					me->BossYell("Break yourselves upon my body. Feel the strength of the earth!", 21921);

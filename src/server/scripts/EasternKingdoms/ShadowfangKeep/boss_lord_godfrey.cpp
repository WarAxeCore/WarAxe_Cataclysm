...
public:
	...
	struct boss_lord_godfreyAI : public BossAI
	{
		...
		void EnterCombat(Unit* pWho)
		{
			...
			instance->SetBossState(DATA_LORD_GODFREY, IN_PROGRESS);
			if (pWho->GetTypeId() == TYPEID_PLAYER && pWho->getFaction() == ALLIANCE)
			{
				me->MonsterYell(SAY_AGGRO_ALLIANCE, 0, 0);
			}
			if (pWho->GetTypeId() == TYPEID_PLAYER && pWho->getFaction() == HORDE)
			{

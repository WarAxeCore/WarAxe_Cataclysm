...
public:
	...
	struct boss_drahga_shadowburnerAI : public ScriptedAI
	{
		...
		{
			me->BossYell("I will burn you from the inside out!", 18610);
		...
		{
			me->BossYell("You should have come better prepared!", 18612);
		}
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_INVOCATION_OF_FLAME:
					if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true))
					{
						...
						me->BossYell("BY FIRE BE... BURNED!", 18619);
					}
	...
	struct npc_drahga_valionaAI : public ScriptedAI
	{
		...
		void UpdateAI(const uint32 diff)
		{
			...
			if (me->HealthBelowPct(30) && stage == 0)
			{
				...
				me->BossYell("I will not die for you, Drahga.", 18524);
				return;
			}

...
public:
	...
	struct boss_erudaxAI : public ScriptedAI
	{
		...
		{
			me->BossYell("The darkest days are still ahead!", 18638);
		...
		{
			me->BossYell("Flesh for the offering!", 18640);
		}
		...
		void UpdateAI(const uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_SHADOW_GALE:
					me->BossYell("F'lakh ghet! The shadow's hunger cannot be sated!", 18644);
				...
				case EVENT_ADDS:
					...
					me->BossYell("Come, suffering... Enter, chaos!", 18646);
					if (FacelessPortalStalker)
					{

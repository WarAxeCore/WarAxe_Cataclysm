...
bool Unit::CanCastWhileWalking(uint32 spell_id)
{
    ...
    AuraEffectList const& auras = GetAuraEffectsByType(SPELL_AURA_CAST_WHILE_MOVING);
...
void Unit::RemoveAurasWithInterruptFlags(uint32 flag, uint32 except)
{
    ...
    for (AuraApplicationList::iterator iter = m_interruptableAuras.begin(); iter != m_interruptableAuras.end();)
    {
        ...
        if ((aura->GetSpellInfo()->AuraInterruptFlags & flag) && (!except || aura->GetId() != except)
            && !(flag & AURA_INTERRUPT_FLAG_MOVE && HasAuraTypeWithAffectMask(SPELL_AURA_CAST_WHILE_MOVING, aura->GetSpellInfo())))
        {
            uint32 removedAuras = m_removedAurasCount;
            RemoveAura(aura);
            if (m_removedAurasCount > removedAuras + 1)
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                ...
                case 39446:
                {
                    ...
                    target = this;
                    if (roll_chance_i(10))
                        ToPlayer()->Say("This is Madness!", LANGUAGE_UNIVERSAL); // TODO: It should be moved to database, shouldn't it?
                    break;
                }
        ...
        case SPELLFAMILY_PALADIN:
        {
			...
			if (dummySpell->Id == 85512)
			{
				...
				if (victim->GetAura(53563, GetGUID()))
				{
					target = this;
					break;
				}
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (trigger_spell_id)
    {
		...
		case 94288:
		case 94289:
		if (victim == this) // Check if we are healing ourselves if so di not proc.
			return false;
		break;
...
bool Unit::isSpellCrit(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    ...
    AuraEffectList const& critAuras = victim->GetAuraEffectsByType(SPELL_AURA_MOD_CRIT_CHANCE_FOR_CASTER_SPELL);
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura)
{
	...
	DamageInfo damageInfo2 = DamageInfo(actor, actionTarget, damage, procSpell, procSpell ? SpellSchoolMask(procSpell->SchoolMask) : SPELL_SCHOOL_MASK_NORMAL, SPELL_DIRECT_DAMAGE);
	...
	ProcEventInfo eventInfo = ProcEventInfo(actor, actionTarget, target, procFlag, 0, 0, procExtra, NULL, &damageInfo2, &healInfo);

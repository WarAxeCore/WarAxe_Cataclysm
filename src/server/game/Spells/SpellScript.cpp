...
bool AuraScript::_Validate(SpellInfo const* entry)
{
	...
	for (std::list<EffectProcHandler2>::iterator itr = OnEffectProc2.begin(); itr != OnEffectProc2.end(); ++itr)
		if (!(*itr).GetAffectedEffectsMask(entry))
	...
	for (std::list<EffectProcHandler2>::iterator itr = AfterEffectProc2.begin(); itr != AfterEffectProc2.end(); ++itr)
		if (!(*itr).GetAffectedEffectsMask(entry))
...
AuraScript::EffectProcHandler2::EffectProcHandler2(AuraEffectProc2FnType effectHandlerScript, uint8 effIndex, uint16 effName)
	: AuraScript::EffectBase(effIndex, effName)
{
	_EffectHandlerScript2 = effectHandlerScript;
}

void AuraScript::EffectProcHandler2::Call(AuraScript* auraScript, AuraEffect const* aurEff, ProcEventInfo& eventInfo)
{
	(auraScript->*_EffectHandlerScript2)(aurEff, eventInfo);
}
...
bool AuraScript::_IsDefaultActionPrevented()
{
    switch (m_currentScriptState)
    {
		...
		case AURA_SCRIPT_HOOK_EFFECT_PROC2:

...
BattlegroundTP::BattlegroundTP()
{
	...
	StartMessageIds[BG_STARTING_EVENT_FIRST] = LANGUAGE_BG_TP_START_TWO_MINUTES;
	StartMessageIds[BG_STARTING_EVENT_SECOND] = LANGUAGE_BG_TP_START_ONE_MINUTE;
	StartMessageIds[BG_STARTING_EVENT_THIRD] = LANGUAGE_BG_TP_START_HALF_MINUTE;
	StartMessageIds[BG_STARTING_EVENT_FOURTH] = LANGUAGE_BG_TP_HAS_BEGUN;
...
void BattlegroundTP::PostUpdateImpl(uint32 diff)
{
	if (GetStatus() == STATUS_IN_PROGRESS)
	{
		...
		if (GetStartTime() >= 27 * MINUTE * IN_MILLISECONDS) ///< End of game - Verify score
		{
		...
		else if (GetStartTime() > uint32((_minutesElapsed + 1) * MINUTE * IN_MILLISECONDS) + 2 * MINUTE * IN_MILLISECONDS)
		{
			++_minutesElapsed;
			...
			case BG_TP_FLAG_STATE_ON_GROUND: ///< Flag is on the ground waiting someone to click
			{
				...
				if (_flagsDropTimer[team] < 0)
				{
					...
					if (GameObject* obj = GetBgMap()->GetGameObject(_droppedFlagGUID[team]))
						...
						sLog->outError( "BattlegroundTP: An error has occurred in PostUpdateImpl: Unknown dropped flag GUID: %u", GUID_LOPART(_droppedFlagGUID[team]));
				...
				if (_flagDebuffState <= 5) ///< Cast on player Focused Assaultupposed to maintain number of stacks for debuff     
				{
					for (uint8 team = TEAM_ALLIANCE; team <= TEAM_HORDE; ++team)
						if (Player* player = ObjectAccessor::FindPlayer(_flagKeepers[team]))
							...
							sLog->outError( "BattlegroundTP: Can't find flag keeper with GUID: %u, in team: %s", _flagKeepers[team], team == TEAM_ALLIANCE ? "ALLIANCE" : "HORDE");
				}
...
void BattlegroundTP::StartingEventOpenDoors()
{
	...
	UpdateWorldState(BG_TP_FLAG_CAPTURES_ALLIANCE, 0);
	UpdateWorldState(BG_TP_FLAG_CAPTURES_HORDE, 0);
}
...
bool BattlegroundTP::SetupBattleground()
{
	...
	{
		sLog->outError("BatteGroundTP: Failed to spawn some objects. Battleground not created!");
		return false;
	}
	...
	for (uint32 i = TP_GRAVEYARD_START_ALLIANCE; i < TP_MAX_GRAVEYARDS; ++i)
	{
		...
		if (grave)
		{
			...
			{
				sLog->outError("BatteGroundTP: Failed to spawn spirit guide id: %u. Battleground not created!", grave->ID);
				return false;
			}
		}
		else
		{
			sLog->outError("BatteGroundTP: Failed to load spirit guide id: %u. Battleground not created!", grave->ID);
			return false;
		}
	}
...
void BattlegroundTP::EventPlayerDroppedFlag(Player* source)
{
	...
	{
		sLog->outError( "BattlegroundTP: An error have occured in EventPlayerDroppedFlag, player: %u who carried the flag is not the flag keeper: %u.", source->GetGUID(), _flagKeepers[team ^ 1]);
		return;
	}
	...
	/// Send message to all
	SendMessageToAll(team == TEAM_ALLIANCE ? LANGUAGE_BG_TP_DROPPED_HF : LANGUAGE_BG_TP_DROPPED_AF, team == TEAM_ALLIANCE ? CHAT_MSG_BG_SYSTEM_HORDE : CHAT_MSG_BG_SYSTEM_ALLIANCE, source);
}
	...
	if (source->IsWithinDistInMap(target_obj, 10)) ///< Check if target is in distance with flag
	{
		...
		case BG_TP_FLAG_STATE_ON_BASE: ///< If opposite team has flag in base
		{
			if (BgObjects[BG_TP_OBJECT_A_FLAG + (team ^ 1)] == target_obj->GetGUID()) ///< If clicked object is what we want
			{
				...
				SendMessageToAll(team == TEAM_ALLIANCE ? LANGUAGE_BG_TP_PICKEDUP_HF : LANGUAGE_BG_TP_PICKEDUP_AF, team == TEAM_ALLIANCE ? CHAT_MSG_BG_SYSTEM_ALLIANCE : CHAT_MSG_BG_SYSTEM_HORDE, source);
			...
			_droppedFlagGUID[team] == target_obj->GetGUID())
		{
			...
			SendMessageToAll(team == TEAM_ALLIANCE ? LANGUAGE_BG_TP_RETURNED_AF : LANGUAGE_BG_TP_RETURNED_HF, team == TEAM_ALLIANCE ? CHAT_MSG_BG_SYSTEM_ALLIANCE : CHAT_MSG_BG_SYSTEM_HORDE, source);
...
void BattlegroundTP::EventPlayerCapturedFlag(Player* source)
{
	...
	SendMessageToAll(team == TEAM_ALLIANCE ? LANGUAGE_BG_TP_CAPTURED_HF : LANGUAGE_BG_TP_CAPTURED_AF, team == TEAM_ALLIANCE ? CHAT_MSG_BG_SYSTEM_ALLIANCE : CHAT_MSG_BG_SYSTEM_HORDE, source);
			...
			if (_bothFlagsKept)
			{
				...
				for (uint8 teamTemp = TEAM_ALLIANCE; teamTemp <= TEAM_HORDE; ++teamTemp)
				{
					if (Player* pl = ObjectAccessor::FindPlayer(_flagKeepers[team]))
					{
						...
						sLog->outError( "BattlegroundTP: An error has occurred in RemovePlayer: player with GUID: %u haven't been found. (_bothflagsKept is TRUE).", _flagKeepers[team]);
				}
...
void BattlegroundTP::RespawnFlag(uint32 team, bool captured)
{
	...
	if (captured)
	{
		...
		SendMessageToAll(LANGUAGE_BG_TP_F_PLACED, CHAT_MSG_BG_SYSTEM_NEUTRAL);
	}
	else
	{
		SpawnBGObject(BG_TP_OBJECT_A_FLAG + team, RESPAWN_IMMEDIATELY);
		SendMessageToAll(LANGUAGE_BG_TP_ALLIANCE_FLAG_RESPAWNED + team, CHAT_MSG_BG_SYSTEM_NEUTRAL);
	}
